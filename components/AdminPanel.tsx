...
export default function AdminPanel() {
  ...
  const getRequestStatusBadge = (status: string) => {
    switch(status) {
      case "approved":
        return <span className="px-2 py-2 bg-green-500/20 text-green-400 rounded-full text-[10px] sm:text-xs">✓ Approved</span>;
      case "rejected":
        return <span className="px-2 py-2 bg-red-500/20 text-red-400 rounded-full text-[10px] sm:text-xs">✗ Rejected</span>;
      case "contacted":
        return <span className="px-4 py-2 bg-blue-500/20 text-blue-400 rounded-full text-[10px] sm:text-xs">🤝 Contacted</span>;
      default:
