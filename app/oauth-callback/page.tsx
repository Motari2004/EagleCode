  ...
  useEffect(() => {
    ...
    if (error) {
      ...
      if (type === "not_signed_up" || decodedError.includes("No account found")) {
        ...
        // Start countdown - will redirect after 10 seconds
        let count = 10;
        setCountdown(count);
...
if (token && !hasProcessed.current) {
  ...
  try {
    ...
    const userData = {
      id: payload.user_id,      // Database UUID from backend
      email: payload.email,
      name: payload.name,
    ...
    // Only call login if user is not already set
    if (!user) {
      login(userData);
    }
  } catch (e) {
  ...
  // DIRECT REDIRECT - bypass handleRedirect
  setTimeout(() => {
